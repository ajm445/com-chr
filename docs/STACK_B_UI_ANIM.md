...
### 6.2 SpeechBubble (`SpeechBubble.tsx`) — 대사 시스템

#### 레벨별 대사 해금

대사에 `minLevel` 필드를 부여하여, 펫 레벨이 올라갈수록 말할 수 있는 대사가 점진적으로 해금된다.

| 레벨 구간 | 테마 | 특징 |
|---|---|---|
| Lv.1 | 갓 태어남 | `...`, `?` 등 최소한의 반응만 (~12개) |
| Lv.2~3 | 말문이 트임 | 기본 감정 표현, 주변 인식 |
| Lv.4~5 | 자아 형성 | 테크 용어, 유머, 풍부한 표현 |
| Lv.6~7 | 철학자/해커 | 자아 인식, 고급 테크 유머 |
| Lv.8~9 | 달인 | 완성형 대사, 자부심 |
| Lv.10~14 | 자각기 | 자신이 프로그램이라는 인식 + 주인 교감 시작 |
| Lv.15~19 | 탐구기 | OS/인터넷/바깥 세계에 대한 호기심 |
| Lv.20~29 | 현자기 | 깊은 사유, 존재론적 대사, 주인과의 유대 |
| Lv.30~49 | 초월기 | 달관, 메타 발언, 강한 교감 |
| Lv.50~99 | 전설기 | 극한의 존재감, 시적 표현 |
| Lv.100+ | 신화기 | 초월적 존재, 궁극의 유대 |

레벨 상한 없음. 고레벨일수록 주인과의 교감 대사 비중이 높아진다.

```typescript
interface Line {
  text: string
  condition?: (state: PetState) => boolean  // 스탯 조건
  weight?: number                            // 가중치 (기본 1)
  tag?: LineTag                              // 표정 연동
  minLevel?: number                          // 해금 레벨 (기본 1)
}
```

#### 대사 선택 로직 (`pickLine`)

1. `LINES` 배열에서 `state.level >= minLevel` 필터
2. `condition(state)` 통과하는 대사만 풀에 포함
3. 가중치 기반 랜덤 선택
...
표시 시간: 3초. 상호작용 대사("냠냠", "헤헤", "뽀득")는 우선 표시.
