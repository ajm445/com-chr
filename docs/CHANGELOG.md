...
### Fixed
- 랜덤 말풍선이 떠 있을 때 상호작용(먹이/놀이/청소)을 하면 상호작용 말풍선이 랜덤 말풍선을 덮어버리던 문제 수정. 이제 랜덤 말풍선이 위로 부드럽게 밀려 올라가고, 원래 자리에 상호작용 말풍선이 표시된다.

### Added
- 말풍선 페이드아웃 애니메이션(`bubble-out`): opacity 감소 + 살짝 위로 뜨면서 blur 처리로 "스르륵" 사라지는 효과.

### Changed
- 상태창(hover 시 Lv/스탯) 위치가 화면에 떠 있는 말풍선 개수(0/1/2)에 맞춰 자동으로 그 위로 쌓이도록 보정.
- `SpeechBubble` 내부를 랜덤 / 상호작용 두 개의 독립 슬롯 구조로 리팩토링(각자 lifecycle, stale closure 방지).
